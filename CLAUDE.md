...
- **Phase:** Phase 3 T3 in progress. Chunks 15-94 complete 2026-07-07. 2276 total city pages built (75 T1 + 300 T2 + 1901 T3). T2 COMPLETE.
- **City pages built:** 2276. T1 complete (75). T2 complete (300). T3 in progress: 1901 built of ~5,000 target.
- **State hub pages:** 51 (all states plus DC have a hub; 8 were added 2026-07-02, see SEO-BUILD-PLAN-2026-07-02.md).
- **Town pages built:** 0.
- **Templates:** 5 (A to E), all built. Router in `src/components/templates/`. Rotation: A=467, B=454, C=452, D=440, E=463 (all within 15-25% target).
- **Next block:** Chunk 95, T3 continues, rotation at C.
- **Deploy:** Always push directly to `main`. deploy.yml fires automatically. Site goes live on every push. Exception: chunks 93-94 (2026-07-07) were pushed to branch `claude/gracious-euler-22bzcj` instead, because that session's harness assigned a designated branch and forbade pushing elsewhere; deploy has not fired for that batch pending a manual merge to main. See MEMORY.md.
