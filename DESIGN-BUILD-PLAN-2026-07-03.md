...
- 2026-07-03 (Sonnet), Block D6: Verify, document, deploy, hand back.
  - D6.1: Full `npm run build` passes (2,245 routes, up from 2,241 before Block D0's 4
    new service pages). Ran `npm run check:words` (still failing, pre-existing content
    debt from the separate SEO remediation track, unaffected by this design pass, not
    this block's job to fix) and `npm run check:headings` (passes, worst variant 28.7%,
    unchanged, confirming the design changes did not touch content or heading rotation).
  - D6.2: Updated the three mandatory docs in the same pass: `BUILD-PLAN.md` (fixed a
    stale "Where we are" summary left over from before chunk 87 and added a session-log
    row for this design block, clearly marked as not a city-page chunk), `build_state.json`
    (added a `design_remediation_note` field alongside the existing SEO one, corrected
    the stale `services_count` from 25 to 27), and `MEMORY.md` (added a Current State
    entry for 2026-07-03, corrected the services count reference). Also corrected
    `CLAUDE.md`'s CURRENT STATUS section and last-updated date, since it carries the
    same "keep accurate" rule.
  - D6.3: Hosting note for Gareth. The plain "skateboarding" 404 page some URLs were
    showing is Hostinger's own fallback for a path that was never built into the static
    export, not a broken page on our side (`src/app/not-found.tsx` is fully branded).
    Block D0 removed the internal links that pointed at non-existent service pages, so
    normal site navigation should no longer hit it. If Gareth wants a mistyped or
    external URL to also show the branded 404 instead of Hostinger's default, Hostinger
    needs to be told to use `/404.html` (already generated by the static export) as its
    custom error document. This is a hosting-panel setting, not a code change, so it is
    handed to Gareth to action rather than done here.
  - D6.4: Live review links posted in chat (see chat reply following this changelog
    entry).
