...
| 3.1 / 4 (combined) | Bulk execution against the rubric | Sonnet, only against the Opus rubric | In progress: 16 of ~1,065 thin records done. All 3 Arizona T1 and all 13 California T1 records complete. ~60 T1 remain, then T2, then T3. See the pacing note below. |
| 4.3 | Wire word-count guard into prebuild once backlog hits zero | Sonnet | Not started |

**Pacing note (2026-07-02):** the bulk content work is far larger than it first looked. Each record needs 2 to 4 new sections, each fact-checked against that city's own data and word-counted to actually clear the floor, which in practice took multiple drafting passes per record even with the rubric in hand. This session completed 16 T1 records (all of Arizona and California) across several sustained batches. At that rate, clearing the full backlog (~60 more T1, then ~294 T2, then ~697 T3) is realistically a multi-session effort, not a single continuous run. Continue in state-sized or tier-sized batches, verifying and committing after each, exactly as done so far, rather than attempting the remainder in one pass. Suggested next states for T1: Colorado, DC, Florida, Georgia, Hawaii, Idaho, Illinois, Indiana (alphabetical continuation).
