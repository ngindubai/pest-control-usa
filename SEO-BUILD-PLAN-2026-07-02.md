...
| 3.1 / 4 (combined) | Bulk execution against the rubric | Sonnet, only against the Opus rubric | In progress: 58 of ~1,051 thin records done (2026-07-03, third update). All T1 records complete through New Jersey, New Mexico, New York, North Carolina, Ohio, and Oklahoma. 18 T1 remain (Oregon, Pennsylvania, Tennessee, Texas, Virginia, Wisconsin), then T2 (~273), then T3 (~718). See the pacing note below. |
| 4.3 | Wire word-count guard into prebuild once backlog hits zero | Sonnet | Not started |

**Pacing note (2026-07-02, updated 2026-07-03 three times):** the bulk content work is far larger than it first looked. Each record needs 2 to 4 new sections, each fact-checked against that city's own data and word-counted to actually clear the floor, which in practice took multiple drafting passes per record even with the rubric in hand. The first session completed 16 T1 records (all of Arizona and California). A second session completed 13 more (Colorado, DC, Florida, Georgia, Hawaii, Idaho, Illinois, Indiana). A third session completed 15 more (Kansas, Kentucky, Louisiana, Maryland, Massachusetts, Michigan, Minnesota, Missouri, Nebraska, Nevada). A fourth session completed 14 more in one continuous batch: Newark and Jersey City (NJ), Albuquerque (NM), New York City and Buffalo (NY), Charlotte, Raleigh, and Greensboro (NC), Columbus, Cleveland, Cincinnati, and Toledo (OH), Oklahoma City and Tulsa (OK), finishing every T1 record from New Jersey through Oklahoma. At that rate, clearing the full backlog (~18 more T1, then ~273 T2, then ~718 T3) is realistically a multi-session effort, not a single continuous run. Continue in state-sized or tier-sized batches, verifying and committing after each, exactly as done so far, rather than attempting the remainder in one pass. Suggested next states for T1 (alphabetical continuation, the final stretch of T1): Oregon, Pennsylvania, Tennessee, Texas (8 cities, the largest single-state group left), Virginia, Wisconsin. After that, T1 is complete and the effort moves to T2. Style note (carried forward and actively applied in the fourth session): each closing paragraph this batch was deliberately varied, contrast pairs ("versus"), "one thing explains most of it, but not all of it," "two very different mechanisms," rather than repeating any single closing device across every record. Two same-state pairs (Newark/Jersey City and Oklahoma City/Tulsa) were deliberately checked against each other to confirm they don't read as templated with the city name swapped. Worth continuing that discipline as the corpus grows past 1,000 touched records.
...
### Session 2026-07-04 (Opus): Trust-claims softening + technical SEO hardening (second external audit)

A second external SEO audit (pasted by Gareth) flagged unverified trust claims, sitemap lastmod noise, a robots block on Next assets, fake social profiles, missing legal pages, and missing logo/OG assets. Gareth reviewed the majors with pros and cons and chose: soften the trust claims, remove the dead social links, add Privacy + Terms, generate placeholder logo/OG, and (separately) remove the fabricated testimonials until real ones exist. Work done:

- **Trust claims softened (FTC + E-E-A-T risk).** Removed the invented rating (4.9), review count (12,847), BBB accreditation, technician count (500+), homes-treated (50,000+ / 250K+ / 2.4M), years-in-business, "$2M liability", NPMA/QualityPro/GreenPro/ISO/OSHA certifications, and "since 2010 / Est. 2011" scale claims from all user-facing UI. Replaced with defensible signals only (licensed and insured, 50 states + DC, 24/7 emergency, same-day, free inspections, satisfaction guarantee). `src/config/site.ts` is now the single source of truth and holds no fabricated metrics or social URLs. Touched: `site.ts`, `lib/schema.ts`, `components/ui/TrustBadges.tsx`, `components/sections/HomepageHero.tsx`, `HomepageStats.tsx`, `HomepageReviews.tsx`, `components/layout/Header.tsx`, `Footer.tsx`, and pages `services/[slug]`, `services`, `locations/[slug]`, `locations`, `emergency`, `about`, `faq`, `residential`, `careers`, `reviews`. City data files were left untouched (their "50,000"/"4.9" strings are population and prose, not claims).
- **Fabricated testimonials removed (Gareth's call).** The 6 invented named testimonials on the homepage, the 3 on `residential`, and the 12 on `reviews` (plus the fake ratings histogram and "verified via Google & BBB" badges) were removed. `HomepageReviews.tsx`, `residential`, and `reviews` rebuilt as honest service-commitment sections; `/reviews` now sets honest expectations and invites real reviews. Re-introduce Review/aggregateRating schema only once real, consented, on-page reviews exist.
- **Licensing claim softened.** The absolute "every technician holds a current state license and carries full liability insurance, we verify licensing and conduct background checks on all field staff" (state pages + FAQ) was softened to "we work with licensed, insured local technicians who meet your state's requirements; ask for their details before work begins," which fits the service-area/lead-gen model. Also removed a banned "comprehensive" from the FAQ.
- **robots.ts:** added an explicit `Allow: /_next/static/` so Googlebot can fetch CSS/JS for rendering, while still disallowing the rest of `/_next/`.
- **sitemap.ts:** stopped stamping every URL with `new Date()` at build time (told Google every page changed on every deploy, which Google may then ignore). `lastModified` is now omitted until a real per-record updatedAt exists. Added the two new legal routes.
- **schema.ts:** dropped the `sameAs` array (social profiles were not live), added real `logo` (`/logo.png`) and `image` (`/og-default.png`).
- **Social links removed** from the footer and the org schema (`Footer.tsx`, `site.ts`).
- **Legal pages added:** `src/app/privacy-policy/page.tsx` and `src/app/terms-of-service/page.tsx` (conventional, plain-English templates; Gareth to have a lawyer review). Linked in the footer legal nav and added to the sitemap.
- **Brand assets generated:** `public/logo.png` (600x160 wordmark, used for schema logo) and `public/og-default.png` (1200x630 social share image), rendered as committed static PNGs via the bundled `next/og` so they work with static export. Wired into `layout.tsx` openGraph/twitter metadata. Removed the obsolete `keywords` metadata.

Verified with `npm run build` (clean, 2247 pages, ESLint passed) and a full `out/` sweep confirming zero fabricated tokens (12,847 / 2.4 Million / 250K+ / BBB Accredited / NPMA / verified reviews / 4.9 rating) remain in the rendered HTML. `check:headings` unaffected.

**Left for Gareth / follow-up (not code):** submit the sitemap and inspect sample URLs in Google Search Console; run PageSpeed/CrUX for Core Web Vitals; have a lawyer review the Privacy and Terms pages; collect real reviews and real social profiles before either is re-introduced. The About-page founding narrative (2011 Dallas timeline) was left as previously agreed in the design audit and is a candidate for a future honesty review if those specifics are not real.

### Session 2026-07-03 (Sonnet): Block 4 bulk, batch 4, 14 more T1 records (T1 complete through Oklahoma)

Continued the Block 4 bulk pass against the Opus rubric (Step 3.1/4), following the prior session's suggested alphabetical continuation. Expanded 14 T1 records across 6 states, each to comfortably clear the 800-word floor, using only facts already established in that record's own `pestProfile`, `climateDriver`, `localHook`, `prevention`, and `faqs`, no new pest claims introduced:

- `src/data/cities/new-jersey.ts`: newark (257 to 865 words), jersey-city (241 to 814).
- `src/data/cities/new-mexico.ts`: albuquerque (267 to 859).
- `src/data/cities/new-york.ts`: new-york-city (195 to 830), buffalo (260 to 886).
- `src/data/cities/north-carolina.ts`: charlotte (192 to 874), raleigh (220 to 813), greensboro (254 to 922).
- `src/data/cities/ohio.ts`: columbus (195 to 848), cleveland (242 to 847), cincinnati (247 to 890), toledo (250 to 879).
- `src/data/cities/oklahoma.ts`: oklahoma-city (244 to 912), tulsa (260 to 911).

Every new section drafted in a scratch file first and word-counted precisely with the counting script before insertion. Each record's final sections checked against the banned-vocabulary list and the dash ban before insertion (all clean). Verified per-record with `npm run check:words`, then the full `npm run check:words`, `npm run check:headings` (still passes, 28.7% worst variant, unaffected), and `npm run build` (clean, 2245 static paths generated) after the full batch.

**Style discipline applied from the prior sessions' flagged notes:** varied the closing-paragraph device across all 14 records this batch rather than repeating any single phrasing, used direct "versus" contrasts, "one thing explains most of it, but not all of it" framings, and structural splits (old-versus-new, flats-versus-hills, quiet-versus-visible risk) instead. Newark and Jersey City (both dense NJ port-adjacent cities with nearly identical pest profiles) were deliberately given different emphasis angles, port/rail/transit-hub for Newark versus high-rise-vertical/commuter/redevelopment for Jersey City, and Oklahoma City and Tulsa (same state, same core pest list including brown recluse) were checked against each other and given different contrast pairs to avoid reading as templated with the city name swapped.

58 of ~1,051 thin records now done. All T1 records complete from New Jersey through Oklahoma. 18 T1 remain (Oregon, Pennsylvania, Tennessee, Texas, Virginia, Wisconsin), then T2 (~273), then T3 (~718). See the updated pacing note above for the next suggested batch.

### Session 2026-07-03 (Sonnet): Block 4 bulk, batch 3, 15 more T1 records

Continued the Block 4 bulk pass against the Opus rubric (Step 3.1/4), following the prior session's suggested alphabetical continuation. Expanded 15 T1 records across 8 states, each to comfortably clear the 800-word floor, using only facts already established in that record's own `pestProfile`, `climateDriver`, `localHook`, `prevention`, and `faqs`, no new pest claims introduced:

- `src/data/cities/kansas.ts`: wichita (250 to 861 words).
- `src/data/cities/kentucky.ts`: louisville (247 to 868), lexington (236 to 869).
- `src/data/cities/louisiana.ts`: new-orleans (216 to 860).
- `src/data/cities/maryland.ts`: baltimore (246 to 855).
- `src/data/cities/massachusetts.ts`: boston (245 to 865).
- `src/data/cities/michigan.ts`: detroit (230 to 865).
- `src/data/cities/minnesota.ts`: minneapolis (190 to 856), saint-paul (273 to 849).
- `src/data/cities/missouri.ts`: kansas-city (265 to 869), st-louis (250 to 852).
- `src/data/cities/nebraska.ts`: omaha (252 to 865), lincoln (257 to 858).
- `src/data/cities/nevada.ts`: las-vegas (218 to 902), henderson (272 to 888).

Every new section drafted in a scratch file first and word-counted precisely with the counting script before insertion, matching the discipline from the prior batch. Each record's final sections checked against the banned-vocabulary list and the dash ban before insertion (all clean, one instance of "landscape" caught and rewritten to a literal, non-figurative term out of caution). Verified per-record with `npm run check:words`, then `npm run check:headings` (still passes, 28.7% worst variant, unaffected) and `npm run build` (clean) after the full batch.

**Style discipline applied from the prior session's flagged note:** actively varied the closing-paragraph device across all 15 records this batch rather than repeating the same "X is really the thread that connects" phrasing from the previous session. Used direct comparisons ("what X explains and what it doesn't"), season-count framing ("two seasons, not four"), and structural contrasts (uniform master-planned design versus mixed older housing) instead. Two cities in the same state (Kansas City and St. Louis, both sitting at river confluences) got deliberately different closing framings, rivers-versus-climate for Kansas City, old-housing-stock-versus-river for St. Louis, specifically to avoid the two records reading like a template with the city name swapped.

44 of ~1,051 thin records now done. 32 T1 remain, then T2 (~273), then T3 (~718). See the updated pacing note above for suggested next states.

### Session 2026-07-03 (Sonnet): Block 4 bulk, batch of 13 T1 records

Continued the Block 4 bulk pass against the Opus rubric (Step 3.1/4), following the prior session's suggested alphabetical continuation. Expanded 13 T1 records across 8 states, each to comfortably clear the 800-word floor (targeted roughly 850 to 900 words), using only facts already established in that record's own `pestProfile`, `climateDriver`, `localHook`, `prevention`, and `faqs`, no new pest claims introduced:

- `src/data/cities/colorado.ts`: denver (214 to 904 words), colorado-springs (260 to 883), aurora-co (254 to 892).
- `src/data/cities/district-of-columbia.ts`: washington (244 to 873).
- `src/data/cities/florida.ts`: miami (187 to 867), jacksonville (210 to 855), tampa (259 to 851), orlando (238 to 871).
- `src/data/cities/georgia.ts`: atlanta (210 to 876).
- `src/data/cities/hawaii.ts`: honolulu (254 to 878).
- `src/data/cities/idaho.ts`: boise (240 to 868).
- `src/data/cities/illinois.ts`: chicago (200 to 873).
- `src/data/cities/indiana.ts`: indianapolis (225 to 869).

Each intro was checked against the answer-first rule; most already led answer-first and were left alone, Denver's and Aurora's openings were lightly rewritten to name the dominant pests in sentence one, matching the rubric's rotation of opening shapes rather than converging on one phrasing. Every new section drafted in a scratch file first and word-counted precisely with a small counting script before insertion, to avoid the previous session's estimation drift (guessing a section at 250 to 300 words when it actually ran 120 to 180). Each record's final sections checked against the banned-vocabulary list and the dash ban before insertion (all clean). Verified per-record with `npm run check:words`, then a full `npm run check:headings` (still passes, 28.7% worst variant, unaffected since only `sections[]` body content was added, not template-rotated headings) and `npm run build` (2,245 routes, clean) after the full batch.

**Style note (not a blocking issue, flagged for future batches):** several closing paragraphs in this batch lean on a similar rhetorical device ("X is really the thread/detail that connects/explains..."). Checked with a grep across all 8 files touched this batch: it did not produce literal duplicate text, every sentence is distinct and grounded in that city's own facts, but it is a stylistic tic worth varying more deliberately across the roughly 1,000 records still remaining, so the corpus does not read as though every page ends the same way.

47 T1 records remain below floor, then T2 (~273), then T3 (~718). See the updated pacing note above for suggested next states.
