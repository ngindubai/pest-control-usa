...
| 3.1 / 4 (combined) | Bulk execution against the rubric | Sonnet, only against the Opus rubric | In progress: 29 of ~1,051 thin records done (2026-07-03 update). All 3 Arizona T1, all 13 California T1, and 13 more T1 records across Colorado (Denver, Colorado Springs, Aurora), DC (Washington), Florida (Miami, Jacksonville, Tampa, Orlando), Georgia (Atlanta), Hawaii (Honolulu), Idaho (Boise), Illinois (Chicago), and Indiana (Indianapolis) complete. 47 T1 remain, then T2 (~273), then T3 (~718). See the pacing note below. |
| 4.3 | Wire word-count guard into prebuild once backlog hits zero | Sonnet | Not started |

**Pacing note (2026-07-02, updated 2026-07-03):** the bulk content work is far larger than it first looked. Each record needs 2 to 4 new sections, each fact-checked against that city's own data and word-counted to actually clear the floor, which in practice took multiple drafting passes per record even with the rubric in hand. The first session completed 16 T1 records (all of Arizona and California). A second session completed 13 more T1 records in one continuous batch (Colorado's 3 cities, DC, all 4 Florida T1 cities, Atlanta, Honolulu, Boise, Chicago, Indianapolis), following the alphabetical continuation this note suggested last time. At that rate, clearing the full backlog (~47 more T1, then ~273 T2, then ~718 T3) is realistically a multi-session effort, not a single continuous run. Continue in state-sized or tier-sized batches, verifying and committing after each, exactly as done so far, rather than attempting the remainder in one pass. Suggested next states for T1 (alphabetical continuation): Kansas, Kentucky, Louisiana, Maryland, Massachusetts, Michigan, Minnesota, Missouri, Nebraska, Nevada. Style note from the 2026-07-03 batch: watch for over-reliance on a single closing-paragraph rhetorical pattern ("X is really the thread/detail that connects/explains...") across cities, it did not create literal duplicate text this batch (each sentence was distinct and city-specific, confirmed by grep), but it is a stylistic tic worth varying more deliberately in future batches so 1,000+ pages do not all end the same way.
