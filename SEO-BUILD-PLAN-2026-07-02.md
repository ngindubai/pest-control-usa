...
| 3.1 / 4 (combined) | Bulk execution against the rubric | Sonnet, only against the Opus rubric | In progress: 44 of ~1,051 thin records done (2026-07-03, second update). All 3 Arizona T1, all 13 California T1, plus 28 more T1 records across Colorado, DC, Florida, Georgia, Hawaii, Idaho, Illinois, Indiana, Kansas, Kentucky, Louisiana, Maryland, Massachusetts, Michigan, Minnesota, Missouri, Nebraska, and Nevada complete. 32 T1 remain, then T2 (~273), then T3 (~718). See the pacing note below. |
| 4.3 | Wire word-count guard into prebuild once backlog hits zero | Sonnet | Not started |

**Pacing note (2026-07-02, updated 2026-07-03 twice):** the bulk content work is far larger than it first looked. Each record needs 2 to 4 new sections, each fact-checked against that city's own data and word-counted to actually clear the floor, which in practice took multiple drafting passes per record even with the rubric in hand. The first session completed 16 T1 records (all of Arizona and California). A second session completed 13 more (Colorado, DC, Florida, Georgia, Hawaii, Idaho, Illinois, Indiana). A third session in the same day completed 15 more in one continuous batch: Wichita (KS), Louisville and Lexington (KY), New Orleans (LA), Baltimore (MD), Boston (MA), Detroit (MI), Minneapolis and Saint Paul (MN), Kansas City and St. Louis (MO), Omaha and Lincoln (NE), Las Vegas and Henderson (NV), following the alphabetical continuation this note suggested last time. At that rate, clearing the full backlog (~32 more T1, then ~273 T2, then ~718 T3) is realistically a multi-session effort, not a single continuous run. Continue in state-sized or tier-sized batches, verifying and committing after each, exactly as done so far, rather than attempting the remainder in one pass. Suggested next states for T1 (alphabetical continuation, the last stretch of T1): New Jersey, New Mexico, New York, North Carolina, Ohio, Oklahoma, Oregon, Pennsylvania, Tennessee, Texas, Virginia, Wisconsin. Style note (carried forward and actively applied in the third session): each closing paragraph this batch was deliberately varied, direct comparisons, "what X explains and doesn't," "two seasons not four," rather than repeating the same "thread that connects" phrasing from the prior batch. Worth continuing that discipline as the corpus grows past 1,000 touched records.
