...
export const districtOfColumbiaCities: CityLocation[] = [
  {
    ...
    sections: [
      ...
      {
        heading: "German versus American cockroaches, two different fights",
        body: "German and American cockroaches split DC's roach problem into two distinct fights. German cockroaches favor the kitchens and bathrooms of apartment buildings, where shared walls and plumbing chases let a single unit's infestation spread to neighboring units before anyone notices. American cockroaches, the larger species, are more common in basements and around sewer access points across the District, drawn in by the same aging sewer infrastructure that sustains the city's rat population. Because both species run year-round in a climate this humid, cockroach control in DC is treated as an ongoing service rather than a seasonal one, with sanitation and moisture control doing as much work as any direct treatment. A single visible German cockroach in a kitchen at night is rarely an isolated find, since the species is nocturnal and hides in cracks and voids during the day, so a sighting typically means an established population rather than one stray insect.",
      },
      {
        heading: "Where DC's mosquito pressure actually comes from",
        body: "Mosquito pressure in DC comes from three distinct sources that all feed the same season. The Potomac River corridor and the Anacostia floodplain provide extensive standing-water habitat along the city's edges, while Rock Creek Park cuts a wooded, shaded corridor of breeding habitat straight through the middle of the District. Culex mosquitoes, the species capable of carrying West Nile virus, are active through the full April to October season, with pressure peaking in the hottest, most humid stretch of July and August. Because the habitat driving mosquito numbers is largely public parkland and riverfront rather than anything a single property owner controls, the most effective individual step is eliminating standing water on the property itself: clogged gutters, saucers under potted plants, and any low spot in the yard that holds water after rain.",
      },
      {
        heading: "Why the rowhouse block, not just the house, matters",
        body: "DC's rowhouse architecture is really what ties several of these pests together. The narrow alleys behind rowhouse blocks in neighborhoods like Adams Morgan, Capitol Hill, and Shaw serve double duty as trash storage and a sheltered travel corridor, which is exactly the kind of harborage that sustains a dense rat population and lets it move easily between properties. Those same older buildings, many with wood in direct contact with soil and crawl spaces without a proper vapor barrier, are the housing stock most exposed to subterranean termites. And the shared walls that let a Norway rat travel unit to unit do the same job for German cockroaches. A rowhouse block is, in effect, one connected pest environment rather than a set of separate homes, which is why coordinated, building-wide pest response tends to outperform one unit treating alone.",
      },
      {
        heading: "Why DC pest control runs year-round",
        body: "Taken together, DC's pest pressure does not really have an off-season. Rats and cockroaches run year-round regardless of weather, termites swarm each spring but stay active through fall, stink bugs arrive each September in a concentrated invasion, and mosquitoes hold a long season from April through October. That is the real basis for treating DC pest control as continuous management rather than a single visit: with this many species active in overlapping windows, a plan built around the calendar catches problems earlier and costs less than responding to each infestation as it appears. The practical difference this makes shows up most clearly in a rowhouse block that treats pest control unit by unit instead of building-wide. A single apartment can clear its German cockroaches and still see the population return within weeks if the unit next door, sharing the same wall voids and plumbing chases, never gets treated. The same logic applies to rat exclusion along a shared alley: sealing one property's trash storage does little if the rest of the block leaves harborage in place. DC's density is what makes the city's pest pressure so persistent, and it is also exactly why a coordinated, building-wide or block-wide approach consistently outperforms any single household acting alone.",
      },
    ],
    prevention: [
